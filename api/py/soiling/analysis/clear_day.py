...
from ..config import SystemConfig, ClearDayResult
...
class ClearDayDetector(ABC):
    @abstractmethod
    def classify_day(self, day: date, intervals: pd.DataFrame, rain_mm: float = 0.0) -> ClearDayResult: ...

    @abstractmethod
    def compute_model_profile(self, day: date) -> tuple[np.ndarray, pd.DatetimeIndex]: ...

    def classify_all(self, energy_15min: pd.DataFrame, precip: pd.DataFrame) -> list[ClearDayResult]:
    ...
    def _apply_baseline_scale(self, results: list[ClearDayResult]) -> list[ClearDayResult]:
        ...
        for i, r in enumerate(results):
            if r.classification == "clear":
                ...
                if np.isfinite(bs) and bs > 0:
                    ...
                    updated.append(
                        ClearDayResult(
                            date=r.date, classification=r.classification,
                            clear_fraction=r.clear_fraction, fit_score=r.fit_score,
                            estimated_clean_kwh=round(est_clean, 2), actual_kwh=r.actual_kwh,
                            model_scale=round(float(bs), 4), n_intervals=r.n_intervals,
...
class CurveMatchDetector(ClearDayDetector):
    def __init__(
        ...
        clear_r2_threshold: float = 0.97, cloudy_r2_threshold: float = 0.50,
        min_daylight_intervals: int = 8, model_power_threshold_w: float = 200,
    ):
        self.config = config
    ...
    def classify_day(self, day: date, intervals: pd.DataFrame, rain_mm: float = 0.0) -> ClearDayResult:
        ...
        model_energy = model_day * (15 / 60)
        actual_energy = actual_day
        ...
        scale = np.median(ratio[clear_mask]) if n_clear >= self.min_daylight else median_ratio
        ...
        return ClearDayResult(
            date=day, classification=classification,
            clear_fraction=round(clear_fraction, 3), fit_score=round(fit_score, 4),
            estimated_clean_kwh=round(estimated_clean_kwh, 2), actual_kwh=round(actual_kwh, 2),
            model_scale=round(float(scale), 4), n_intervals=int(n_valid),
            n_clear_intervals=int(n_clear),
        )

    def _align_actual(self, intervals: pd.DataFrame, model_times: pd.DatetimeIndex) -> np.ndarray:
        if intervals.empty:
        ...
        actual_df = pd.DataFrame({"energy_wh": intervals["energy_wh"].values}, index=ts_naive)
