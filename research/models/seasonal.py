...
Fits a smooth 3-harmonic curve to the cleanest clear-day production values,
creating a "what a clean system produces on day X" ceiling for each day of year.
"""
...
def seasonal_curve(doy: np.ndarray, A, B, phi1, C, phi2, D, phi3) -> np.ndarray:
    """Three-harmonic seasonal model: captures asymmetric summer plateau."""
    return (
        A
...
def fit_seasonal_envelope(daily: pd.DataFrame) -> tuple[np.ndarray, pd.Series]:
    """
    Fit a smooth seasonal envelope to the cleanest clear-day production.

    Strategy: bin clear days by DOY (10-day bins), take P95 of each bin,
    then fit a smooth 3-harmonic curve through those P95 points.

    Args:
        daily: DataFrame with columns 'date', 'energy_kwh', 'is_clear'

    Returns:
        params: fitted curve parameters (7 values)
        envelope: Series indexed by DOY (1-366) with envelope kWh values
    """
    clear = daily[daily["is_clear"]].copy()
    clear["doy"] = pd.to_datetime(clear["date"]).dt.dayofyear

    # P95 per 10-day bin — the cleanest days define the envelope
    n_bins = 36
