...
Detects cleaning events from soiling ratio recovery patterns, distinguishing
between rain-induced cleaning and manual cleaning. Uses noise-adaptive
thresholds and requires sustained recovery to filter out noise.
"""
...
def detect_cleaning(daily: pd.DataFrame) -> pd.DataFrame:
    """
    Detect cleaning events from SR recovery patterns.

    Physics constraint: during dry periods (no rain), soiling only
    accumulates — SR can only decrease. Any upward jump that is NOT
    explained by rain must be a manual cleaning, and must be sustained.

    Args:
        daily: DataFrame with columns 'sr_raw', 'is_usable', 'is_dry', 'rain_mm'

    Returns:
        DataFrame with added columns: 'cleaning', 'clean_mag', 'event_type'
    """
    daily = daily.copy()
    ...
    # Step 1: Compute smoothed SR for candidate detection
    ...
    # Step 2: Estimate noise level from day-to-day SR variation
    ...
    # Step 4: Consolidate nearby candidates
    consolidated = []
    if candidates:
        group = [candidates[0]]
    ...
    # Step 5: Classify events by cause
...
def classify_event(max_rain: float, total_rain: float) -> str:
    """Classify a cleaning event by its likely cause based on rainfall."""
